...
def convert(
    ...
    try:
        dw = dhdr["CRDELT1"]  # wavelength step
    except:
        dw = dhdr["CDELT1"]  # wavelength step
    wunit = dhdr["CUNIT1"]  # wavelength unit
    wtype = "WAVE"  # type spectra

    # define the dimensions of the spaxel array
    Nx, Ny, x0, y0, dx, dy = getspaxdim(
        data, phdr, sky_bundles, expansion_factor=expansion_factor
    )
    ...
    cube.header.update(CD1_1=-dx / 3600.0)
    cube.header.update(CD2_2=dy / 3600.0)
    ...
    ### OPTIMIZACIÓN: Pre-calculamos el arreglo de longitudes de onda una sola vez fuera del bucle
    lambda_arr = w0 + dw * numpy.arange(0, Nw, 1)
    ...
    for ispec in range(len(data)):
        ...
        if not (phdr["FIB" + fib_str + "_B"] in sky_bundles):
            try:
                ...
                if "start_sp" in locals():
                    print(
                        "Warning! FIB"
                        + fib_str
                        + "W1 and W2 information missing in header. Assuming previous fiber wavelength coverage."
                    )
                else:
                    end_sp = Nw
                    start_sp = 1
                    print(
                        "Warning! FIB"
                        + fib_str
                        + "W1 and W2 information missing in header. Assuming default wavelength coverage."
                    )
            ...
            if end_sp != start_sp:
                ...
                xpos = (phdr["FIB" + fib_str + "_x"] + 5.0) * cte.PLATESCALE
                ypos = (phdr["FIB" + fib_str + "_y"] + 5.0) * cte.PLATESCALE
                ix = int(round((xpos - x0), 3) / dx)
                iy = int(round((ypos - y0), 3) / dy)

                ### OPTIMIZACIÓN 1: Inserción y conversión de unidades vectorizada (Slicing)
                limit = min(end_sp, Nwspec)
                spec_slice = spec[start_sp:limit]

                if keep_units:
                    cube.data[start_sp:limit, iy, ix] = spec_slice
                else:
                    lambda_slice = lambda_arr[start_sp:limit]
                    cube.data[start_sp:limit, iy, ix] = (
                        spec_slice * 3.00e-5 / (lambda_slice**2)
                    )
            else:
                end_sp = Nwspec
    ...
    cube_rebin.header.remove("CRVAL1", ignore_missing=True)
    cube_rebin.header.remove("CUNIT1", ignore_missing=True)
    cube_rebin.header.remove("CTYPE1", ignore_missing=True)
    cube_rebin.header.remove("CDELT1", ignore_missing=True)
    cube_rebin.header.remove("CRPIX2", ignore_missing=True)
    cube_rebin.header.remove("CRVAL2", ignore_missing=True)
    cube_rebin.header.remove("CDELT2", ignore_missing=True)
    cube_rebin.header.remove("CTYPE2", ignore_missing=True)
    ...
    cube_rebin.header.update(CD1_1=-dx * nbin / 3600.0)
    cube_rebin.header.update(CD2_2=dy * nbin / 3600.0)
    ...
    # Extraemos la sección útil (ignorando bordes que no completan un bin)
    data_to_rebin = cube.data[:, :Ny_trunc, :Nx_trunc]

    # Reshape a 5D y suma sobre los ejes de los bloques espaciales
    cube_rebin.data = data_to_rebin.reshape(Nw, Ny // nbin, nbin, Nx // nbin, nbin).sum(
        axis=(2, 4)
    )

    if writeout != None:
        cube_rebin.writeto(writeout, overwrite=overwrite)
        print(f"Cubo guardado exitosamente en: {writeout}")
